...
public class ElementTestService {
    ...
    public void selectCacheTest() {
        ...
        /* 최초요청 시에는 시간이 걸리지만 그 이후 에는 캐싱된 데이터를 불러오기 때문에 속도가 빠르다. */
        for(int i = 0; i < 10; i++) {
            ...
            System.out.println("수행시간 : " + inteval + "(ms)");
        }
        sqlSession.close();
    }
    ...
    public void selectResultMapConstructorTest() {
        ...
        List<MenuDTO> menuuList = mapper.selectResultMapConstructortest();
        for (MenuDTO menu : menuuList) {
            System.out.println(menu);
        }
        sqlSession.close();
    }
